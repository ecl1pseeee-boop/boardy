# Отчет по лабораторной работе: Настройка виртуальной машины


### 1. Параметры виртуальной машины
#### 01-vm-settings.png
![Параметры ВМ](screenshots/01-vm-settings.png)

### 2. Консоль виртуальной машины
#### 02-vm-console.png
![Консоль ВМ](screenshots/02-vm-console.png)

### 3. Информация о системе
#### 03-system-info.png
![Информация о системе](screenshots/03-system-info.png)

### 4. IP-адрес
#### 04-ip-addr.png
![IP-адрес](screenshots/04-ip-addr.png)

### 5. Открытые порты
#### 05-ports.png
![Открытые порты](screenshots/05-ports.png)

### 6. Статус SSH
#### 06-ssh-status.png
![Статус SSH](screenshots/06-ssh-status.png)

### 7. Порт SSH
#### 07-ssh-port.png
![Порт SSH](screenshots/07-ssh-port.png)

### 8. Список пользователей
#### 08-users.png
![Список пользователей](screenshots/08-users.png)

### 9. Создание нового пользователя
#### 09-new-user and 10-user-check.png
![Создание пользователя](screenshots/09-new-user\ and\ 10-user-check.png)

### 10. Содержимое корня (/)
#### 11-root-tree.png
![Корневая директория](screenshots/11-root-tree.png)

### 11. Домашняя директория
#### 12-home-tree.png
![Домашняя директория](screenshots/12-home-tree.png)

### 12. Права доступа
#### 13-permissions.png
![Права доступа](screenshots/13-permissions.png)

### 13. Изменение прав (chmod)
#### 14-chmod.png
![Изменение прав](screenshots/14-chmod.png)

### 14. Установленные пакеты
#### 15-packages.png
![Пакеты](screenshots/15-packages.png)

### 15. Запущенные сервисы
#### 16-services.png
![Сервисы](screenshots/16-services.png)

### 16. Топ процессов
#### 17-top-processes.png
![Топ процессов](screenshots/17-top-processes.png)

### 17. Количество процессов
#### 18-process-count.png
![Количество процессов](screenshots/18-process-count.png)

### 18. Крупные файлы
#### 19-big-files.png
![Крупные файлы](screenshots/19-big-files.png)

### 19. Файлы отчета
#### 20-report-files.png
![Файлы отчета](screenshots/20-report-files.png)

