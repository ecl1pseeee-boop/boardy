...
async def create_comment(post_id: int, data: CommentIn,
    ...
    comment_id = await db_insert(
        '''INSERT INTO comments (post_id, author_id, author_name, body)
           VALUES (%s, %s, %s, %s)''', post_id, int(user['sub']), data.author_name, data.body
    )
    comment = {
        'id': comment_id,
        'post_id': post_id,
...
async def update_comment(comment_id: int, data: CommentUpdate,
    ...
    # Самая безопасная проверка: приводим всё к int, убирая пробелы
...
async def delete_comment(comment_id: int,
    ...
    # Логируем для отладки в journalctl
    print(f"DEBUG DELETE: existing={existing}, author_id={existing.get('author_id')}, user_sub={user.get('sub')}", flush=True)
